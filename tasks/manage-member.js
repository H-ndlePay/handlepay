...
task("manage-member", "Create a member and add handles; optional ETH payment")
  ...
  .addOptionalParam("pay", "ETH amount to send to handle(s)", undefined, types.string)
  .addFlag("payEach", "If set, pay each handle instead of just the first")
  .addVariadicPositionalParam("handles", "Handles as platform:username", [], types.string)
  .setAction(async ({ wallet, registry, pay, payEach, handles }, hre) => {
    if (!wallet) throw new Error("--wallet is required");
    if (!handles.length) throw new Error("Provide at least one handle: platform:username");

    const [signer] = await hre.ethers.getSigners();
    console.log("Signer:", signer.address);

    const r = await hre.ethers.getContractAt("HandleRegistry", registry, signer);
    ...
    if (memberId === 0) {
      ...
      await r.createMember.staticCall(wallet); // v6 static call
      const tx = await r.createMember(wallet);
      await tx.wait(1);
      memberId = Number(await r.walletToMemberId(wallet));
      console.log(`Member created with id=${memberId}`);
    } else {
      console.log(`Member already exists with id=${memberId}, skipping create.`);
    }

    // add handles
    for (const h of handles) {
      const [platform, username] = (h || "").split(":");
      if (!platform || !username) throw new Error(`Bad handle "${h}", use platform:username`);
      ...
      if (taken === 0n) {
        ...
        await r.addHandle.staticCall(memberId, platform, username); // v6 static call
        const tx = await r.addHandle(memberId, platform, username);
        await tx.wait(1);
        console.log("  added ✅");
      } else if (Number(taken) === memberId) {
        console.log(`Handle ${platform}:${username} already on this member, skipping.`);
      } else {
        console.log(`Handle ${platform}:${username} taken by memberId=${taken.toString()}, skipping.`);
      }
    }
